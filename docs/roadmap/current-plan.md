...
- 当前阶段：阶段 1，本地记录闭环收尾
- 当前基线：`master` / `6d0a320aeda7642e759cb69a6e5467b973624ea5`
...
| CNG-109 | 代码已提交，验收未冻结 | 最新完整证据来自 API 35 模拟器，`qemu=1`；是否必须补物理真机由负责人决定 |
...
当前工作区还有两个不得顺手处理的既有状态：`pubspec.lock` 有未提交依赖解析差异；`cognote-agent-artifacts/` 位于仓库内且未跟踪。品牌改名不授权处理它们，也不包含 Dart package、数据库、Android applicationId 或仓库路径迁移。

## 2. 先决策，再继续交付

下列事项需要负责人明确结论。它们不阻塞文档审计，但阻塞发布、暂存和后续任务验收：
...
| GATE-03 | CNG-109 是否接受模拟器证据 | 若“真实设备”指物理机，则补 API 35 真机；否则书面接受模拟器并冻结结论 | 决定 CNG-109 能否关闭 |
...
- 根据 GATE-03 补物理真机证据，或记录接受模拟器的负责人结论；
- 记录 `pubspec.lock` 是否与证据基线一致，不在该任务中夹带依赖调整。

关闭条件：证据可复现、设备等级被接受、遗留风险有负责人结论。
...
3. force-stop 后 Observation、LocalAsset、墓碑和 Outbox 数量/内容一致；
